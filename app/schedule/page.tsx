...
export default function SchedulePage() {
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FA] pb-20 relative">
      <div className="fixed right-6 top-1/2 -translate-y-1/2 z-40 hidden xl:flex flex-col gap-4 bg-white/80 backdrop-blur-md p-3 rounded-full shadow-2xl border border-gray-200">
            ...
            <button
              ...
              className={`w-10 h-10 rounded-full flex items-center justify-center text-xs font-bold transition-all duration-300 relative group ${
                isToday
                  ...
                  : 'bg-gray-100 text-gray-500 hover:bg-[#21D0B8] hover:text-white'
              }`}
            >
              {day.short}
            ...
            {[1, 2].map(i => (
              <div key={i} className="animate-pulse">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-6">
                  {[1, 2, 3, 4, 5, 6].map(k => (
                    <div key={k} className="aspect-2/3 bg-gray-200 rounded-2xl"></div>
                  ))}
                </div>
              </div>
            ))}
            ...
            return (
              <div
                ...
                className={`rounded-3xl transition-all duration-500 ${
                  isToday
                    ? 'bg-white p-8 shadow-[0_10px_40px_-10px_rgba(33,208,184,0.15)] border border-[#21D0B8]/40 relative overflow-hidden'
                    : 'p-2'
                }`}
              >
                {isToday && (
                  <div className="absolute top-0 right-0 w-64 h-64 bg-[#21D0B8]/5 rounded-full blur-3xl -translate-y-1/2 translate-x-1/3 pointer-events-none"></div>
                )}
                ...
                <div className="flex items-center gap-4 mb-8 relative z-10">
                  ...
                  {!isToday && <div className="h-px flex-1 bg-gray-200 ml-4"></div>}
                </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-6 relative z-10">
                  {animeForDay.map(anime => (
                    ...
                    >
                      ...
                      <div>
                        <h3 className="font-bold text-gray-800 text-sm line-clamp-1 group-hover:text-[#21D0B8] transition-colors">
                          {anime.title}
                        </h3>
